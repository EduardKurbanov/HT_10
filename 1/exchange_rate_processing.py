...
def __daterange(start_date):
    for n in range(int((datetime.now() - start_date).days + 1)):
        yield start_date + timedelta(n)


def __get_currency_exchange_rate_by_date(date_in):
...
def __get_currency_by_date_range(start_date, currency: str):
    try:
        data_list: list = []
        rate: str = None
        currency_string: str
        ...
        if currency in currency_list:
            for single_date in __daterange(start_date):
                data: dict = __get_currency_exchange_rate_by_date(single_date.strftime("%d.%m.%Y"))
...
def __currency_converter(arg_1: str = "", arg_2: str = "", arg_3: float = 1):
    try:
        data_list: list = []
        currency_string: str
        rate_1: str = "1"
        ...
        data: dict = __get_currency_exchange_rate_by_date(datetime.now().strftime("%d.%m.%Y"))
        date_value: str = data["date"]
        temp_val = data["exchangeRate"]
        # print(temp_val)
        if len(temp_val) == 0:
            ...
            data: dict = __get_currency_exchange_rate_by_date(date.now().strftime("%d.%m.%Y"))
            date_value: str = data["date"]
            temp_val = data["exchangeRate"]
                        ...
                        if course_abbreviation in ["USD", "EUR", "RUR", "CHF", "GBP", "PLZ", "SEK", "XAU", "CAD"]:
                            ...
                            if day in range(1, 31 + 1):
                                ...
                                if mounth in range(1, 12 + 1):
                                    ...
                                    year = int(input(f"3. enter the month in the range -> 2014–{datetime.now().year} -> "))
                                    if year <= datetime.now().year:
                                        start_date = datetime(year, mounth, day)
                                        __get_currency_by_date_range(start_date, course_abbreviation)
                        ...
                        if base_unit in ["UAH", "USD", "EUR", "RUB", "CHF", "GBP", "PLZ", "SEK", "XAU", "CAD"]:
                            ...
                            if secondary_unit in ["UAH", "USD", "EUR", "RUB", "CHF", "GBP", "PLZ", "SEK", "CAD"]:
                                ...
                                if isinstance(float(base_currency_number),(float, int)):
                                    resul = __currency_converter(base_unit, secondary_unit, float(base_currency_number))
                                    print(
...
# get_currency_by_date_range(start_date, "USD")
# exchange_rates()
# __currency_converter()
# currency_console(True)
